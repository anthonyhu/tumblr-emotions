...
Pictures are valuable to accurately infer the emotion expressed by a user. For instance, happy photos might contain sunny landscapes while sad pictures might contain darker colors. To analyse the images, we'll use convolutional neural networks, which achieve state-of-the-art performances in many visual recognition tasks.

\subsection{Transfer Learning}
Training a convolutional network from scratch can be difficult as a large amount of data is needed and plenty of different architectures and hyperparameters need to be tried before finding a decent model. To circumvent this issue, we can take advantage of the pre-trained Inception network [cite] that learned to recognise images through near 1.2M training examples from the ImageNet dataset, and with a deep architecture (22 layers) that took weeks to train on multiple GPUs.
